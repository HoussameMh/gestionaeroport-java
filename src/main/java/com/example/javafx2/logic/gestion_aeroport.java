...
public class gestion_aeroport {
    ...
    public void planifier(Vol var1) throws AvionIndisponibleException {
        // Le constructeur Vol vérifie que l'avion est "disponible" et change son état à "envol"
        // Donc si le Vol a été créé avec succès (id_vol > 0), cela signifie que l'avion était disponible
        // On accepte donc les avions en "envol" qui ont un id_vol > 0
        
        // Vérifier que le Vol est valide (a un avion et un id_vol > 0)
        ...
        // Vérifier que l'avion n'est pas déjà utilisé dans un autre vol
        for (Vol v : this.vols) {
        ...
        // Le constructeur a déjà vérifié la disponibilité et changé l'état à "envol"
        // Donc on peut ajouter le vol
        this.vols.add(var1);
    ...
    public Piste autoriser_descente_(Vol var1) throws PisteNonDisponibleException {
        // Vérifier et mettre à jour la disponibilité de toutes les pistes d'abord
        ...
        while (var2.hasNext()) {
            ...
            // Vérifier que la piste est vraiment disponible (y compris le temps d'occupation)
            pisteCandidate.verifierEtMettreDisponible();
            
            // Vérifier si cette piste convient
    ...
    public Piste Parker(Vol var1) {
        ...
        Piste var4;
        // Libérer la piste actuelle (escale) où se trouve l'avion
        while(var3.hasNext()) {
        ...
        // Chercher une piste de parking disponible
        var3 = this.pistes.iterator();
        while(var3.hasNext()) {
            var4 = (Piste)var3.next();
            // Vérifier que la piste est vraiment disponible
            ...
            if (var4.getEst_dispo() && var4.getType().equals("parking") && 
                ...
                // Pour le parking, temps_occup = 0 car pas de limite de temps
                var4.occuper(0.0);
    ...
    public Piste autoriser_depart(Vol var1) {
        ...
        Piste var4;
        // Libérer la piste actuelle (parking) où se trouve l'avion
        while(var3.hasNext()) {
            var4 = (Piste)var3.next();
            if (var4.getCoordonnees()[0] == var1.getAvion().getCoordonnees()[0] && 
                var4.getCoordonnees()[1] == var1.getAvion().getCoordonnees()[1]) {
        ...
        // Chercher une piste d'escale disponible pour le départ
        var3 = this.pistes.iterator();
        while(var3.hasNext()) {
            var4 = (Piste)var3.next();
            // Vérifier que la piste est vraiment disponible
